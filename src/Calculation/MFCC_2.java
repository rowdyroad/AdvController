...
public class MFCC_2 {
	 ...
	 */
	public MFCC_2(int nnumberOfParameters,
		...
		calculateMelBasedFilterBank(dsamplingFrequency,
									nnumberofFilters,
									nFFTLength);
		m_fft = new FFT_2(m_nFFTLength); //initialize FFT
		initializeDCTMatrix();
		...
		if (m_oisLifteringEnabled) {
			...
			for (int i=0; i<m_nlifteringCoefficient; i++) {
				m_nlifteringMultiplicationFactor[i] = (float) (1.0f + dfactor * Math.sin(dfactor2*(i+1)));
			}
	...
	{
		int hopSize = m_nFFTLength / 2;
		   float[][] mfcc = new  float[(data.length/hopSize)-1][];
		    //process each window of this audio segment
		    for(int i = 0, pos = 0; pos < data.length - hopSize; i++, pos+=hopSize)
		    	mfcc[i] = processWindow(data, pos);		    
		    return mfcc;	    	
	}
	public float[] processWindow(float[] fspeechFrame, int begin) 
	{
		for(int i = 0; i < m_nnumberOfFilters; i++) 
		{			
			final int m_DBFBin_f = m_nboundariesDFTBins[i][0];
			final int m_DBFBin_l = m_nboundariesDFTBins[i][1];			
			m_dfilterOutput[i] = 0.0f;
			final float[] data = (m_ousePowerInsteadOfMagnitude) ? m_fft.calculateFFTPower(fspeechFrame,begin) : m_fft.calculateFFTMagnitude(fspeechFrame,begin);			
			for(int j = m_DBFBin_f, k=0; j<=m_DBFBin_l; j++,k++) 
			{
					m_dfilterOutput[i] += data[j] * m_dweights[i][k];
			}
			m_dfilterOutput[i] = (m_dfilterOutput[i] > m_dminimumFilterOutput)  ?  (float) Math.log(m_dfilterOutput[i]) : m_dlogFilterOutputFloor;
		}
	...
	public float[] getFilterBankOutputs(float[] fspeechFrame, int begin) {
		//use mel filter bank
		...
		for(int i=0; i < m_nnumberOfFilters; i++) {
			...
			if (m_ousePowerInsteadOfMagnitude) {
				float[] fpowerSpectrum = m_fft.calculateFFTPower(fspeechFrame,begin);
				for(int j=m_nboundariesDFTBins[i][0], k=0;j<=m_nboundariesDFTBins[i][1];j++,k++) {
					dfilterOutput[i] += fpowerSpectrum[j] * m_dweights[i][k];
				}
			} else {
				float[] fmagnitudeSpectrum = m_fft.calculateFFTMagnitude(fspeechFrame,begin);
				for(int j=m_nboundariesDFTBins[i][0], k=0;j<=m_nboundariesDFTBins[i][1];j++,k++) {
...
/*
        public static void main(String[] args) {
          ...
          float dsamplingFrequency = 8000.0;
          int nFFTLength = 512;
