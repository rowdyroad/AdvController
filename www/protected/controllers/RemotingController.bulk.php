...
class RemotingController extends CController
{
    ...
    {
        $r = $this->q("select ident from promo_idents")->queryColumn();
        if (count($r) >= 253)
        {
        ...
        while (true)
        {
            ...
            if ($i == 15)
                continue;
            $r = $this->q("select 1 from promo_idents where ident = :ident")->
            bindParam(":ident",$i)->
            queryAll();            
            if (empty($r))
            {
    ...
    public function actionCache()
    {
        ...
        {            
            ...
            $ids = Array();
            $r = $this->q("select *, unix_timestamp(dt_registered) as `timestamp` from promo_idents_events where status = 0 order by dt_registered DESC limit 10")->query();            
            while ($row = $r->read()) {
                extract($row);  
                $this->addEventToCache($promo_idents_event_id, $soas_id, $promo_ident,$probability,$timestamp);
                $ids[] = $promo_idents_event_id;                
            }               
            $this->q("update promo_idents_events set status = 1 where promo_idents_event_id in (".implode(", ",$ids).")")->execute();
            
            $count = $this->q("update events_cache set status = 2 where status = 1 limit 10")->execute();
    ...
    {
        if (!$r = $this->q("select id, length from promo_idents where ident = :ident")->
    ...
    public function actionAddIdentEvent()
    {
        ...
        try
        {
            ...
            $count = preg_match_all('/(\d+),(\d+)(,(\d+))?/', $_REQUEST['events'], $matches);
            if ($count === false)
                throw new Exception('Error parsing input string');

            if ($count === 0)
                throw new Exception('No events were provided');
            $q = "insert into promo_idents_events(dt_registered, soas_id, promo_ident, probability) values ";
            for ($i = 0; $i < $count; ++$i)
            {
                $dt_unix_timestamp = intval($matches[1][$i]);
                $promo_ident = intval($matches[2][$i]);
                $dt_registered =  date('Y-m-d H:i:s', $dt_unix_timestamp);                
                if ($dt_registered[0] == '3') { $dt_registered[0] = '2'; }
                $probability = (!empty($matches[4][$i])) ? intval($matches[4][$i]) : 100;
                $q.="('$dt_registered','$soas_id','$promo_ident','$probability')";
            }        
            $this->q($q)->execute();
            $this->q("update `soas` set `last_heart_beat` = NOW() where `soas_id` = '$eventSoasId'")->execute();
            $response['result'] = 'success';
        }
        catch (exception $e)
        {
